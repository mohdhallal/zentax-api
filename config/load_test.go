...
func TestLoad_ParsesJSONAndSetsEnvName(t *testing.T) {
	payload := map[string]any{
		"app":              map[string]any{"env": EnvDevelopment, "port": 9090, "internalPort": 9091},
	...
	assert.Equal(t, EnvDevelopment, loaded.EnvName)
...
func TestLoad_SetsGlobalCfg(t *testing.T) {
	...
	payload := map[string]any{
		"app":      map[string]any{"env": EnvDevelopment, "port": 8080},
		"database": map[string]any{"url": "postgres://localhost/test"},
	}
