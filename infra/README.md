...
infra/
  ...
  lib/data-stack.ts        ZenTax-<Title>-Data     KMS, RDS Postgres 16, secrets, documents bucket, WORM audit-export bucket (Object Lock, COMPLIANCE), api + migrate ECR, log groups, alarm topic + RDS alarms
...
| Audit-export bucket (WORM) | `zentax-staging-eu-audit-export-<account>-<region>` | `zentax-production-eu-audit-export-…` |
...
- **Data** is everything stateful or expensive to recreate, plus the cell's
  alarm topic and the RDS alarms (they live next to the instance). It also
  holds the one resource that is *impossible* to recreate: the WORM
  audit-export bucket, whose Object Lock cannot be added to an existing bucket
  or removed from this one (see "The WORM audit archive"). The web tier's
  state — its ECR repository, its log group and the origin-verify secret
  CloudFront and the ALB share — lives with the ALB, in the Web stack.
...
| S3 (WORM) | API | `zentax-<cell>-audit-export-<acct>-<region>` — the write-once copy of the audit hash chain (ADR-0008): SSE-KMS (CMK, bucket key), versioned, block-all-public, TLS-only, **S3 Object Lock in COMPLIANCE mode**, default retention **`auditExportRetentionDays` = 1 d**, `RETAIN` and never auto-emptied. Lifecycle expires **nothing**: it only aborts incomplete multipart uploads after 7 d and moves objects to Glacier Instant Retrieval at 90 d. A second bucket-policy `Deny` covers `s3:DeleteObject`/`s3:DeleteObjectVersion`/`s3:BypassGovernanceRetention` for every principal | same, retention **3653 d (10 years)** — see "The WORM audit archive" |
...
| seed | API | the api's whole non-secret block (`APP_ENV`, `DB_*`, `LOG_FORMAT`, `STORAGE_*`, `AUDIT_EXPORT_*`, `MAIL_*`; `CORS_ALLOWED_ORIGINS=https://seed.invalid` — the same config loader validates all of it, seed-admin serves no HTTP, sends nothing and exports nothing; no `PUBLIC_BASE_URL`, it builds no links) | `DB_PASSWORD`, `AUTH_ENCRYPTION_KEY`, `SEED_ADMIN_PASSWORD` ← seed-admin (used when `--password` is absent) |
...
### The WORM audit archive (ADR-0008)

The application audit trail is append-only in Postgres and hash-chained per
tenant, but every one of those guards lives inside the database the same
operator administers. `zentax-<cell>-audit-export-<acct>-<region>` is the
off-box half: the api exports chain segments into it, and once an object is
there nothing in this account can change or remove it.

**COMPLIANCE, not GOVERNANCE — and the difference is the whole point.** S3
Object Lock offers two retention modes:

- **GOVERNANCE** — a locked version can be deleted early by any principal
  holding `s3:BypassGovernanceRetention`. Real protection against accident;
  worth nothing against the privileged insider ADR-0008 names as the threat
  ("a rogue DBA cannot silently rewrite history"). An administrator can turn it
  off.
- **COMPLIANCE** — chosen here. A locked version cannot be deleted or its
  retention shortened by **anyone** until the retention expires: not an
  administrator, not the account root, not AWS Support, not us. There is no
  bypass permission, no override request, no support ticket.

**What it costs, stated plainly.** Every byte written is billable storage for
the whole retention with no way to delete it early. In production that is
**3653 days — ten calendar years** (the top of ADR-0007's 6–10 year statutory
tax-record range; `Math.ceil(10 × 365.25)`, deliberately not 3650, which would
expire two days before the tenth anniversary of the write). A bug that exports
too much, or exports the wrong thing, cannot be cleaned up — only waited out,
paying for it the whole time. Two consequences follow, and both are already
true of what gets exported: the envelope must stay PII-free (ADR-0007/0008 —
actor by UUID, values redacted to presence tokens), because an unlawful field
cannot be erased from here; and the exporter must **batch**, because per-object
costs and the 128 KB minimum billable object size of Glacier Instant Retrieval
are paid for the full ten years too. Sizing: 1 GB/year of chain segments costs
roughly $0.05/month in the first 90 days and ~$0.004/GB-month afterwards —
about **$2–3 over the whole decade per GB written per year**, plus $0.03/GB on
the rare retrieval. The lock is what is expensive to get wrong, not the storage.

**The recovery story.** There is none, by construction, and that is what was
bought. If the wrong thing is written, the remedies are: wait out the
retention; close the AWS account; or destroy the KMS key, which leaves the
objects in place but no longer decryptable. That last path is the one residual
erase route and it is deliberate — ADR-0006 decision 1 puts all data at rest
under the cell CMK, and a `schedule-key-deletion` is a 30-day, CloudTrail-visible
administrative act rather than a silent one. (SSE-S3 would have closed even that
door, at the price of contradicting ADR-0006 and giving up key control over the
archive.) So: **the archive is proof against every principal in the account
except the holder of the CMK, and against them only slowly and visibly.**

**Staging rehearses the mechanism, not the policy** — `auditExportRetentionDays`
is **1** there. The mode, the bucket policy, the api's single-verb grant and the
export path are identical; only the hold is short, because staging holds
synthetic data and a ten-year COMPLIANCE lock on throwaway objects is a bill
nobody can cancel and a bucket `cdk destroy` can never leave clean. Synth
refuses a *production* cell configured under 2192 days (six years), because an
under-locked archive looks completely healthy right up to the audit that asks
for a year that has lapsed.

**The other three properties, and why each is there:**

- **Versioning on** (Object Lock requires it and forbids suspending it): an
  overwrite of an existing key can only add a version beside the locked one, so
  the original stays readable and locked.
- **A lifecycle that expires nothing.** A `NoncurrentVersionExpiration` shorter
  than the lock is *worse* than no rule — it reads like a retention policy and
  cannot fire, because lifecycle deletion of a locked version simply fails; one
  longer than the lock would erase the archive the hour its protection lapsed.
  The only rules here abort incomplete multipart uploads after 7 days (parts are
  not objects, Object Lock does not cover them, and this is the one thing that
  could accumulate unseen — it is also why the api needs no abort permission)
  and transition to Glacier Instant Retrieval at 90 days.
- **`RETAIN` in both cells, never auto-emptied.** An evidence archive is not
  something a stack teardown may take with it, and CDK's `autoDeleteObjects`
  could not empty it anyway — its custom resource deletes object versions, which
  COMPLIANCE refuses.

**What the api may do: put an object. That is the entire list.** The api task
role carries `s3:PutObject` on `<bucket>/*` and nothing else — no delete verb,
no `*Version` verb, no `s3:PutObjectRetention`/`PutObjectLegalHold` (the bucket
stamps the hold itself at write time, so the writer never names it), no
`s3:AbortMultipartUpload` (the lifecycle rule does that cleanup), and no read
(the exporter resumes from the sequence number in its own database). One verb is
genuinely enough: the S3 adapter issues a single `PutObject` per segment, and
`s3:PutObject` would still cover the whole multipart path if a segment ever grew
past the point where the SDK switches. On top of that the bucket policy `Deny`s `s3:DeleteObject`,
`s3:DeleteObjectVersion` and `s3:BypassGovernanceRetention` for *every*
principal — including the CloudFormation execution role, whose `s3:*` on
`zentax-*` would otherwise reach these objects. That Deny is the second lock and
it is revocable (a principal with `s3:PutBucketPolicy` can drop it), which is
exactly the difference between a policy and an immutability mode — and the
reason both are here.

**Pointing the export at it is a required line, not a default.** The api and
seed containers get `AUDIT_EXPORT_ENABLED=true`, `AUDIT_EXPORT_STORAGE_DRIVER=s3`,
`AUDIT_EXPORT_STORAGE_S3_BUCKET` and `AUDIT_EXPORT_STORAGE_S3_REGION`
(`lib/cluster-stack.ts`, `config/auditexport.go`). The **driver** is the
load-bearing one: with it unset the Go service writes the segments to the
*document* store, which is the right default for the self-hosted edition (one
disk, one place to put bytes) and exactly wrong for a cell — that bucket is
deletable by design, so the export would look healthy and be worth nothing as
evidence. `AUDIT_EXPORT_ENABLED` is redundant with the Go default and set
anyway; a compliance control that runs only because someone else's default
happened to be right is one flipped default away from stopping. The seed task
carries the block for the same reason it carries the mail block (one config
loader, and the `s3` driver refuses to boot without a bucket and a region) and
can export nothing: it runs no scheduler and its task role has no S3 verb at all.

The bucket name is a stack **output** (`AuditExportBucketName`) but deliberately
**not** a CloudFormation export: nothing outside this app reads it, and
`lib/exports.ts` must stay byte-identical in both repositories.
...
| WORM audit archive | API | <$1 | <$1 **and rising, forever** | S3 $0.023/GB-month, Glacier IR $0.004/GB-month after 90 d. Small today, but **COMPLIANCE-locked storage cannot be deleted** — production accumulates for 10 years, ~$2–3 per GB written per year over the whole decade. The line only ever grows; see "The WORM audit archive" |
...
**One exception, on purpose: the audit-export bucket is `RETAIN` in staging
too** and is left behind by the destroy. It is Object-Locked in COMPLIANCE
mode, so nothing — not CloudFormation, not an administrator — can empty it
while a hold is live; `autoDeleteObjects` is off for exactly that reason. Its
retention is one day, so from 24 h after the last export it can be emptied and
deleted by hand:

```bash
# once the newest object is more than auditExportRetentionDays (1) old
aws s3 rm s3://zentax-staging-eu-audit-export-<acct>-eu-central-1 --recursive
aws s3api delete-objects --bucket zentax-staging-eu-audit-export-<acct>-eu-central-1 \
  --delete "$(aws s3api list-object-versions --bucket zentax-staging-eu-audit-export-<acct>-eu-central-1 \
      --query '{Objects: Versions[].{Key:Key,VersionId:VersionId}}')"   # and again for DeleteMarkers
aws s3api delete-bucket --bucket zentax-staging-eu-audit-export-<acct>-eu-central-1
```

Two `AccessDenied`s are possible and they mean different things. If the bucket
policy is still attached, its `DenyObjectRemoval` statement refuses the delete
regardless of the hold — remove that statement first (`aws s3api
delete-bucket-policy`), which is a deliberate step and the reason it is a
separate lock from the Object Lock. If the delete is still refused after that,
the version's hold has not lapsed: wait, and do not look for a bypass — there is
none in COMPLIANCE mode. Destroying the staging Data
stack also schedules the CMK for deletion (30-day window), after which anything
still in that bucket is undecryptable ciphertext; that is the intended cleanup
path for a cell whose contents were synthetic.
...
4. Delete the retained resources by hand, in this order, once the snapshots are
   ...
   **The audit-export bucket is not on that list and cannot be.** Its objects
   are Object-Locked in COMPLIANCE mode for **ten years** from each write: no
   principal, including root and AWS Support, can delete them or shorten the
   hold, so the bucket outlives the cell by design (`aws s3 sync` it to the
   customer's or the successor account's archive if it must move; a copy is a
   new object and starts its own hold). The only way to make it *unreadable*
   sooner is destroying the CMK in the step above — which is why that step says
   "after the last object encrypted with it is gone": here, it will not be. If
   the archive must stay readable, keep the key and pay the $1/month. Ten years
   on, when the last hold has lapsed and the bucket may finally go, the
   `DenyObjectRemoval` bucket-policy statement has to be removed first — it
   denies the delete even after Object Lock stops doing so, on purpose.
