-- Revert canonical_tax_keys
BEGIN;

-- The inverse rename of the deploy: the predefined data templates (VAT Return
-- / Corporate Income Tax / Withholding Tax) go back to their fixture-era field
-- ids (outputVat -> f-vat-output, taxLiability -> f-cit-due, ...), and so do
-- the same keys inside the tax data of the instances bound to those templates
-- (directly, or through their workflow task). The map is a bijection, so this
-- restores exactly what the deploy renamed; values, field order, labels, flags
-- and validation are kept. Custom templates and free-form tax data are
-- untouched.
--
-- Same FORCE-RLS bracket as the deploy: the three tables read or written here
-- FORCE row-level security, which binds the OWNER too (the RDS master user the
-- migrate task runs as is an owner, not a superuser), so the tenant policy
-- would silently match zero rows. FORCE is lifted for the duration of this
-- transaction only — the owner then bypasses the policy — and restored before
-- COMMIT; the app role is never affected.
...
INSERT INTO canonical_key_map (old_key, new_key) VALUES
    ('salesTotal',      'f-vat-sales'),
    ('outputVat',       'f-vat-output'),
    ('inputVat',        'f-vat-input'),
    ('netVat',          'f-vat-net'),
    ('profitBeforeTax', 'f-cit-pbt'),
    ('adjustments',     'f-cit-adj'),
    ('taxableIncome',   'f-cit-taxable'),
    ('taxRate',         'f-cit-rate'),
    ('taxLiability',    'f-cit-due'),
    ('whtBase',         'f-wht-base'),
    ('whtRate',         'f-wht-rate'),
    ('whtAmount',       'f-wht-amount');
